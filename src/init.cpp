...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "viog (or specifically: privatesend, instantsend, masternode, keepass, mnpayments, mngovernance)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all VIOG specific functionality (Masternodes, PrivateSend, InstantSend, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:9910"));
    ...
    strUsage += HelpMessageOpt("-instantsenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9910, default: %u)"), nInstantSendDepth));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The VIOG Community Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that VIOG is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (GetArg("-liquidityprovider", 0) > 0) {
        ...
        mapArgs["-anonymizeviogamount"] = "999999";
        LogPrintf("AppInit2 : parameter interaction: -liquidityprovider=%d -> setting -anonymizeviogamount=999999\n", nLiqProvTmp);
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. The VIOG Wallet is shutting down."));
...
#endif
    // Make sure only a single VIOG process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. The VIOG Wallet is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of the VIOG Wallet") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart the VIOG Wallet to complete") << "\n";
    ...
    nAnonymizeViogAmount = GetArg("-anonymizeviogamount", nAnonymizeViogAmount);
    nAnonymizeViogAmount = std::min(std::max(nAnonymizeViogAmount, 2), 999999);
    ...
    LogPrintf("Anonymize VIOG Amount %d\n", nAnonymizeViogAmount);
    ...
    // ********************************************************* Step 11c: update block tip in VIOG modules
    ...
    // ********************************************************* Step 11d: start viog-privatesend thread
